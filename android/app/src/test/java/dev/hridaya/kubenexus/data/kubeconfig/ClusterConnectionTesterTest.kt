...
class ClusterConnectionTesterTest {
    ...
    private class FakeNativeBridge : KubeNexusNativeBridge {
        var pingResult: Result<String> = Result.Success("Cluster ready & healthy (Kubernetes v1.30.0)")
        ...
        override fun createClientWithOptions(rawKubeconfig: String, timeoutSec: Long, insecure: Boolean): Result<client.Client_> =
            Result.Error(AppError.Unknown("mock"))
        override fun listPods(rawKubeconfig: String, namespace: String?): Result<List<String>> = Result.Success(emptyList())
        override fun listPodsWide(rawKubeconfig: String, namespace: String?): Result<List<client.Pod>> = Result.Success(emptyList())
        override fun listNamespaces(rawKubeconfig: String): Result<List<client.Namespace>> = Result.Success(emptyList())
        ...
        override fun explainResource(rawKubeconfig: String, resourceOrKind: String, groupVersion: String): Result<ResourceExplain> =
        ...
        override fun getPodLogs(rawKubeconfig: String, namespace: String, podName: String, container: String?): Result<String> = Result.Success("")
        override fun streamPodLogs(rawKubeconfig: String, namespace: String, podName: String, container: String?, callback: client.LogCallback): Result<Unit> = Result.Success(Unit)
    ...
    fun `testConnection throws exception with diagnostic information when native ping fails`() {
        fakeNativeBridge.pingResult = Result.Error(AppError.Network("TLS handshake failed: certificate untrusted"))
